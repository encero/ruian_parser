...
type StreetQuery struct {
	...
	withAdresses *AddressPlaceQuery
...
func (sq *StreetQuery) QueryAdresses() *AddressPlaceQuery {
	query := &AddressPlaceQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *StreetQuery) Clone() *StreetQuery {
	...
	return &StreetQuery{
		...
		withAdresses: sq.withAdresses.Clone(),
...
// WithAdresses tells the query-builder to eager-load the nodes that are connected to
// the "adresses" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *StreetQuery) WithAdresses(opts ...func(*AddressPlaceQuery)) *StreetQuery {
	query := &AddressPlaceQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withAdresses = query
	return sq
}
...
func (sq *StreetQuery) sqlAll(ctx context.Context) ([]*Street, error) {
	...
	if query := sq.withCities; query != nil {
		...
		ids := make(map[int32]*Street, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(street.CitiesPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, sq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "cities": %w`, err)
		}
		query.Where(city.IDIn(edgeids...))
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.Cities = append(nodes[i].Edges.Cities, n)
			}
		}
	}

	if query := sq.withAdresses; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int32]*Street, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Adresses = []*AddressPlace{}
		}
		var (
			edgeids []int32
			edges   = make(map[int32][]*Street)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(street.AdressesPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, sq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "adresses": %w`, err)
		}
		query.Where(addressplace.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "adresses" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Adresses = append(nodes[i].Edges.Adresses, n)
			}
		}
	}
