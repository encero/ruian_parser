...
// The AddressPlaceFunc type is an adapter to allow the use of ordinary
// function as AddressPlace mutator.
type AddressPlaceFunc func(context.Context, *ent.AddressPlaceMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AddressPlaceFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.AddressPlaceMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AddressPlaceMutation", m)
	}
	return f(ctx, mv)
}
