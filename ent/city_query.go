...
func (cq *CityQuery) Clone() *CityQuery {
	...
	return &CityQuery{
		...
		withStreet: cq.withStreet.Clone(),
...
// WithStreet tells the query-builder to eager-load the nodes that are connected to
// the "street" edge. The optional arguments are used to configure the query builder of the edge.
...
func (cq *CityQuery) sqlAll(ctx context.Context) ([]*City, error) {
	...
	if query := cq.withStreet; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int32]*City, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Street = []*Street{}
		}
		var (
			edgeids []int32
			edges   = make(map[int32][]*City)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(city.StreetPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, cq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "street": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "street" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Street = append(nodes[i].Edges.Street, n)
			}
		}
	}
